 ...
 * that, call {@link CastexSurfaceView#setAspectRatioMode(int)} with
 * {@link CastexSurfaceView#ASPECT_RATIO_PREVIEW} after creating your
 * {@link CastexSurfaceView}. <br />
 *
 */
public class CastexSurfaceView extends android.view.SurfaceView implements Runnable, SurfaceTexture.OnFrameAvailableListener, SurfaceHolder.Callback {
    ...
    private CastexSurfaceManager mViewSurfaceManager = null;

    // The input surface of the MediaCodec
    private CastexSurfaceManager mCodecSurfaceManager = null;

    // Handles the rendering of the SurfaceTexture we got
    // from the camera, onto a Surface
    private CastexTextureManager mTextureManager = null;
    ...
    public CastexSurfaceView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void addMediaCodecSurface(Surface surface) {
        synchronized (mSyncObject) {
            mCodecSurfaceManager = new CastexSurfaceManager(surface,mViewSurfaceManager);
        }
    }
    ...
    public void startGLThread() {
        ...
        if (mTextureManager == null) {
            mTextureManager = new CastexTextureManager();
        }
        if (mTextureManager.getSurfaceTexture() == null) {
            mThread = new Thread(CastexSurfaceView.this);
            mRunning = true;
            mThread.start();
    ...
    public void run() {
        ...
        mViewSurfaceManager = new CastexSurfaceManager(getHolder().getSurface());
